...
class Crypt
{
    ...
    public static function encrypt(mixed $message, string $key): string
    {
        ...
        if (!$length) {
            throw new Exception('SpryPHP: Encryption failure. Cipher Length is invalid.');
        }
     ...
     */
    public static function decrypt(string $message, string $key): mixed
    {
        $encKey  = hash_hmac(self::HASH_ALGO, 'ENCRYPTION', $key, true);
        $authKey = hash_hmac(self::HASH_ALGO, 'AUTHENTICATION', $key, true);
        ...
        $nonceSize  = openssl_cipher_iv_length(self::METHOD) ?: null;

        if (!$nonceSize) {
            throw new Exception('SpryPHP: Decryption failure. Cipher Length is invalid.');
        }
